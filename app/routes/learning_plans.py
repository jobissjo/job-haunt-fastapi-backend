...
from app.schemas.learning_plan import (BaseResponseSchema,
...
@router.get("/")
async def get_learning_plans(service: LearningPlanService = Depends(LearningPlanService),
user_data:UserTokenDecodedData=Depends(CommonService.verify_token_get_user)) -> LearningPlanListResponse:
...
async def create_learning_plan(
    learning_plan: LearningPlanSchema, service: LearningPlanService = Depends(LearningPlanService)
) -> BaseResponseSchema:
    return await service.create_learning_plan(learning_plan)
