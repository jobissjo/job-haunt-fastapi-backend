...
class UserSkillRepository:
    ...
    async def create_user_skill(self, user_skill: UserSkillSchema, user_id: str)->UserSkillResponse:
    ...
    async def get_user_skill_by_id(self, user_skill_id: str)->UserSkillResponse:
        user_skill_response = await self.collection.find_one({"_id": ObjectId(user_skill_id)})
        user_skill_response["_id"] = str(user_skill_response["_id"])
        return user_skill_response
    
    async def update_user_skill(self, user_skill_id: str, user_skill: UserSkillSchema)->UserSkillResponse:
        user_skill_response = await self.collection.update_one({"_id": ObjectId(user_skill_id)}, {"$set": user_skill.model_dump()})
        user_skill_response["_id"] = str(user_skill_response["_id"])
        return user_skill_response
